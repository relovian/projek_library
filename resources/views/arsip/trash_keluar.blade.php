...
@section('title', 'Trash Arsip Keluar')
@section('breadcrumb') Trash / Arsip Keluar @endsection
...
<div class="max-w-[1100px]">
    ...
    <div class="flex items-center justify-between mb-5">
        <div>
            <h1 class="text-xl font-semibold text-[#111827] m-0">Trash Arsip Keluar</h1>
            <p class="text-[13px] text-[#6b7280] mt-1">
                Arsip keluar yang dihapus akan tersimpan di sini. Hanya admin yang bisa memulihkan atau menghapus permanen.
            </p>
        </div>
        <div>
            @php
                $from = request('from', 'keluar');
            @endphp
            @if($from === 'saya_keluar')
                <a href="{{ route('arsip.index', ['tab' => 'saya', 'arsip_id' => 'arsip_keluar']) }}" class="text-[13px] text-[#6b7280] no-underline px-3.5 py-[7px] border border-[#e5e7eb] rounded-lg bg-white"> Kembali</a>
            @else
                <a href="{{ route('arsip.index', ['tab' => 'keluar']) }}" class="text-[13px] text-[#6b7280] no-underline px-3.5 py-[7px] border border-[#e5e7eb] rounded-lg bg-white"> Kembali</a>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white border border-[#e5e7eb] rounded-xl py-12 text-center text-[#9ca3af]">
        ...
        <p class="text-xs mt-1">Tidak ada arsip keluar yang dihapus.</p>
    </div>
            ...
            <tbody>
                ...
                <tr class="border-b border-[#f3f4f6] last:border-b-0">
                    <td class="px-4 py-3 text-[#6b7280] align-middle">
                        ...
                        <div class="text-[11px] text-[#9ca3af] mt-0.5">{{ $item->kode_arsip_keluar }}</div>
                    </td>
                    ...
                    <td class="px-4 py-3 text-[#6b7280] align-middle">
                        <div class="flex gap-2 justify-center">
                            ...
                            @if(auth()->user()->role === 'admin')
                                ...
                                {{-- Pulihkan --}}
                                <form method="POST" action="{{ route('arsip-keluar.restore', $item->id) }}">
                                    @csrf
                                    @method('PATCH')
                                ...
                                @else
                                    <form method="POST" action="{{ route('arsip-keluar.force-delete', $item->id) }}"
