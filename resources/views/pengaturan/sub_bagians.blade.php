        ...
        <table class="w-full border-collapse text-[13.5px]">
            ...
            <tbody>
                ...
                <tr class="border-b border-border transition-colors duration-[.15s] cursor-pointer hover:bg-surface2 last:border-b-0">
                    ...
                    <td class="px-[14px] py-3">{{ $sub->arsips_count }} arsip</td>
                    ...
                    <td class="px-[14px] py-3">
                        <div class="flex gap-1.5">
                            {{-- TOMBOL EDIT --}}
                            <a href="{{ route('pengaturan.sub_bagians', ['edit' => $sub->id]) }}" 
                            ...
                            <form method="POST" action="{{ route('pengaturan.sub_bagians.destroy', $sub) }}"
    ...
    <div class="bg-surface border border-border rounded-[14px] p-6 mb-[15px]" id="formSubBagianCard">
        ...
        <form method="POST" id="formSubBagian" 
              action="@if(request()->has('edit') && $editSubBagian = \App\Models\SubBagian::find(request()->get('edit'))) 
                        {{ route('pengaturan.sub_bagians.update', $editSubBagian) }} 
                      @elseif(old('_method') == 'PUT' && old('sub_bagian_id')) 
                        {{ route('pengaturan.sub_bagians.update', old('sub_bagian_id')) }} 
                      @else 
                        {{ route('pengaturan.sub_bagians.store') }} 
                      @endif">
            @csrf
            ...
            <div class="flex gap-[10px]">
                ...
                <a href="{{ route('pengaturan.sub_bagians') }}" class="px-3 py-[5px] rounded-[6px] text-[12px] font-semibold cursor-pointer border [font-family:inherit] inline-flex items-center no-underline transition-opacity duration-200 hover:opacity-[0.85] bg-surface2 text-hitam border-border px-2 py-4 text-center">Batal</a>
                @endif
                <button type="button" class="px-3 py-[5px] rounded-[6px] text-[12px] font-semibold cursor-pointer border [font-family:inherit] inline-flex items-center no-underline transition-opacity duration-200 hover:opacity-[0.85] bg-surface2 text-hitam border-border px-2 py-4" onclick="resetSubBagianForm()">Reset</button>
            </div>
        </form>
    </div>
</div>
...
function resetSubBagianForm() {
    window.location.href = '{{ route('pengaturan.sub_bagians') }}';
};
</script>
@endpush