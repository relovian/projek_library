                ...
                <tr class="border-b border-border transition-colors duration-[.15s] cursor-pointer hover:bg-surface2 last:border-b-0">
                    ...
                    <td class="px-[14px] py-3">
                        <div class="flex gap-1.5">
                            {{-- TOMBOL EDIT --}}
                            <a href="{{ route('pengaturan.sifat_surat', ['edit' => $sifat->id]) }}" 
                            ...
                            @if($sifat->arsip_count == 0)
                            <form method="POST" action="{{ route('pengaturan.sifat_surat.destroy', $sifat) }}"
    ...
    <div class="bg-surface border border-border rounded-[14px] p-6 mb-[15px]" id="formSifatSuratCard">
        ...
        <form method="POST" id="formSifatSurat" 
              action="@if(request()->has('edit') && $editSifatSurat = \App\Models\SifatSurat::find(request()->get('edit'))) 
                        {{ route('pengaturan.sifat_surat.update', $editSifatSurat) }} 
                      @elseif(old('_method') == 'PUT' && old('sifat_surat_id')) 
                        {{ route('pengaturan.sifat_surat.update', old('sifat_surat_id')) }} 
                      @else 
                        {{ route('pengaturan.sifat_surat.store') }} 
                      @endif">
            @csrf
            ...
            <div class="flex gap-[10px]">
                ...
                <a href="{{ route('pengaturan.sifat_surat') }}" class="px-3 py-[5px] rounded-[6px] text-[12px] font-semibold cursor-pointer border [font-family:inherit] inline-flex items-center no-underline transition-opacity duration-200 hover:opacity-[0.85] bg-surface2 text-hitam border-border px-2 py-4 text-center">Batal</a>
                @endif
...
function resetSifatSuratForm() {
    window.location.href = '{{ route('pengaturan.sifat_surat') }}';
}
</script>
@endpush